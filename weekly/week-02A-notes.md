...
1. Which DOM method is used (in this course) to get a reference to a *single* HTML page element?
  1. What are the possible return values(s) of this method?
...
1. What does `"use strict"` do? See these articles:
  1. https://love2dev.com/blog/javascript-strict-mode/
...
1. Write a function named `addThem`, that takes 2 arguments named `num1` and `num2`, and returns their sum
  1. Now write another function named `addThem2` that works as above, but also gives `num1` and `num2` *default values* of `0`
  1. Now write another function named `addThem3` that works the same as `addThem2`, but is instead declared as an ES6 arrow function
